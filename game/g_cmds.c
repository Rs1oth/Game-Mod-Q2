...
void Cmd_StanceLeft(edict_t *ent)
{
	ent->client->pers.stance = GUARD_LEFT;
	gi.cprintf(ent, PRINT_HIGH, "Guarding Left.\n");
}

void Cmd_StanceRight(edict_t *ent)
{
	ent->client->pers.stance = GUARD_RIGHT;
	gi.cprintf(ent, PRINT_HIGH, "Guarding Right.\n");
}

void Cmd_StanceTop(edict_t *ent)
{
	ent->client->pers.stance = GUARD_TOP;
	gi.cprintf(ent, PRINT_HIGH, "Guarding Top.\n");
	
}

void Cmd_HeavyAttack(edict_t *ent)
{
	int damage = 50;
	int kick = 500;
	vec3_t g_offset;
	...
	g_offset[0] = vec3_origin[0];
	g_offset[1] = vec3_origin[1];
	g_offset[2] = vec3_origin[2];
...
void ClientCommand (edict_t *ent)
{
	...
	else if (Q_stricmp(cmd, "stanceleft") == 0)
		Cmd_StanceLeft(ent);
	else if (Q_stricmp(cmd, "stanceright") == 0)
		Cmd_StanceRight(ent);
	else if (Q_stricmp(cmd, "stancetop") == 0)
		Cmd_StanceTop(ent);
	else if (Q_stricmp(cmd, "heavyattack") == 0)
		Cmd_HeavyAttack(ent);
